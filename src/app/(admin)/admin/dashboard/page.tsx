...
export default function PaginaDashboardAdmin() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 md:py-10">
      <header>
        <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{formatarDataBR(new Date(), "EEEE',' d 'de' MMMM yyyy")}</p>
        <h1 className="mt-2 font-serif text-3xl tracking-tight md:text-4xl" style={{ fontVariationSettings: '"wght" 590' }}>Painel administrativo</h1>
      ...
      <section className="mt-10" aria-labelledby="tendencia-titulo">
        <header className="mb-4 flex items-end justify-between gap-4">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Atividade da rede</p>
            <h2 id="tendencia-titulo" className="mt-1 font-serif text-xl tracking-tight md:text-2xl" style={{ fontVariationSettings: '"wght" 590' }}>Atividade nas últimas 12 semanas</h2>
          </div>
        </header>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-5">
        <section className="lg:col-span-2" aria-labelledby="insights-titulo">
          <header className="mb-4">
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Sinais detectados</p>
            <h2 id="insights-titulo" className="mt-1 font-serif text-xl tracking-tight md:text-2xl" style={{ fontVariationSettings: '"wght" 590' }}>Insights da IA</h2>
          </header>
        ...
        <section className="lg:col-span-3" aria-labelledby="top-escolas-titulo">
          <header className="mb-4 flex items-end justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Engajamento por escola</p>
              <h2 id="top-escolas-titulo" className="mt-1 font-serif text-xl tracking-tight md:text-2xl" style={{ fontVariationSettings: '"wght" 590' }}>Top 10 escolas</h2>
            </div>
          </header>
    ...
    <div className="rounded-xl border border-ia/20 bg-ia-muted p-5">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <p className="font-serif text-base tracking-tight text-foreground" style={{ fontVariationSettings: '"wght" 510' }}>Sinais da semana</p>
        </div>
        ...
        {itens.map((insight, i) => (
          <li key={insight.id ?? `insight-${i}`} className="flex gap-3">
            ...
            <div className="min-w-0">
              {insight.titulo && <p className="font-serif text-sm tracking-tight text-foreground" style={{ fontVariationSettings: '"wght" 510' }}>{insight.titulo}</p>}
